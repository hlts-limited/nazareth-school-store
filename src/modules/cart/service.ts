...
const cartInclude = {
  ...
  variant: { include: { item: { include: {
    category: { include: { parent: true } },
    pileParts: { orderBy: { sortOrder: "asc" as const }, include: { book: { select: { name: true, price: true } } } },
  } } } },
} as const;
...
export async function addToCart(parentId: string, pupilId: string, variantId: string, qty = 1) {
  ...
  const v = await db.itemVariant.findUnique({ where: { id: variantId }, include: { item: { include: { classes: true, inPiles: { include: { pile: true } } } } } });
  if (!v || !v.item.isActive) throw new UserError("That item is no longer available.");
  if (v.item.inPiles.length) throw new UserError(`${v.item.name} is only sold as part of "${v.item.inPiles[0].pile.name}".`);
  ...
  if (v.item.isPile) {
    // One pile per child; no stock check: books that are out of stock are packed later
    if (existing) throw new UserError(`${v.item.name} is already in your cart for ${pupil.firstName}.`);
    await db.cartItem.create({ data: { parentId, pupilId, variantId, qty: 1 } });
...
export async function setQty(parentId: string, cartItemId: string, qty: number) {
  const line = await db.cartItem.findFirst({ where: { id: cartItemId, parentId }, include: { variant: { include: { item: true } } } });
  if (!line) throw new UserError("That item is no longer in your cart.");
  if (qty <= 0) { await db.cartItem.delete({ where: { id: line.id } }); return; }
  if (line.variant.item.isPile) throw new UserError("A pile is bought once per child.");
...
export async function boughtItemIds(pupilId: string) {
  const lines = await db.orderLine.findMany({
    ...
    select: { pileItemId: true, variant: { select: { itemId: true } } },
  });
  // A pile counts as bought once its books are ordered
  return new Set(lines.flatMap((l) => (l.pileItemId ? [l.variant.itemId, l.pileItemId] : [l.variant.itemId])));
}
...
export async function addBooklist(parentId: string, pupilId: string) {
  const pupil = await assertGuardian(parentId, pupilId);
  const required = await db.item.findMany({ where: { isActive: true, inPiles: { none: {} }, classes: { some: { classId: pupil.classId, isCompulsory: true } } }, include: { variants: true } });
  ...
  for (const it of required) {
    ...
    if (!v || (!it.isPile && v.onHand - v.reserved <= 0)) { outOfStock.push(it.name); continue; }
...
export async function booklistProgress(pupilId: string, classId: string) {
  const required = await db.item.findMany({ where: { isActive: true, inPiles: { none: {} }, classes: { some: { classId, isCompulsory: true } } }, select: { id: true, name: true, price: true } });
  const bought = await boughtItemIds(pupilId);
